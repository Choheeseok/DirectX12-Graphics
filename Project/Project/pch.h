...
static uniform_real_distribution<float> urd{ 0.0f, 1.0f };
static default_random_engine dre;

constexpr int FRAME_BUFFER_WIDTH = 1280;
constexpr int FRAME_BUFFER_HEIGHT = 960;
...
inline float RandomFloat() { return(urd(dre)); }
...
namespace Vector3
{
	...
	inline XMFLOAT3 Random()
	{
		return XMFLOAT3(urd(dre), urd(dre), urd(dre));
	}
}
...
namespace Vector4
{
	...
	inline XMFLOAT4 Random()
	{
		return XMFLOAT4(urd(dre), urd(dre), urd(dre), urd(dre));
	}
}
